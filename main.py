...
list_skipped_text = ['Личный кабинет', 'Найти собеседника', 'Отменить', 'Написать в поддержку', 'Завершить чат']
    ...
    if room_id is not None:
        if message.text in list_skipped_text:
            ...
            if message.text:
                ...
                await bot.send_video(room_mate_id, message.video.file_id, caption='Собеседник')
            elif message.sticker:
                await bot.send_sticker(room_mate_id, message.sticker.file_id, caption='Собеседник')
            elif message.audio:
                await bot.send_audio(room_mate_id, message.audio.file_id, caption='Собеседник')
            elif message.game:
                await bot.send_game(room_mate_id, message.gme.file_id, caption='Собеседник')
            elif message.voice:
                await bot.send_voice(room_mate_id, message.voice.file_id, caption='Собеседник')
            elif message.photo:
                await bot.send_photo(room_mate_id, message.photo.file_id, caption='Собеседник')
            elif message.video_note:
                await bot.send_video_note(room_mate_id, message.video_note.file_id, caption='Собеседник')
            elif message.dice:
                await bot.send_dice(room_mate_id, message.dice.file_id, caption='Собеседник')
            elif message.animation:
                await bot.send_animation(room_mate_id, message.animation.file_id, caption='Собеседник')
...
async def connect_users():
    ...
    if queue is None:
        ...
        await bot.send_message(users_id[0], 'Себеседник найден!', reply_markup= await keyboard('exit'))
