...
class SQLite:
    ...
    def get_queue(self):
        return self.cursor.execute("SELECT user_id FROM queue LIMIT 2").fetchall()
    ...
    def get_count_rooms(self):
        return self.cursor.execute("SELECT room_id FROM rooms").fetchall()

    def add_user_in_base(self, user_id, username):
        self.cursor.execute(
            "INSERT INTO users"
            "('user_id', 'username', 'total_spend_time', 'time_register')"
            "VALUES (?,?,?,?)",
    ...
    def add_user_in_queue(self, user_id):
        self.cursor.execute("INSERT INTO queue ('user_id', 'started') VALUES (?,?)", (user_id, datetime.now()))
        self.database.commit()

    def add_new_room(self, room_id, first_user_id, second_user_id):
        self.cursor.execute("INSERT INTO rooms ('room_id', 'first_user_id', 'second_user_id', 'started') VALUES (?,?,?,?)",
